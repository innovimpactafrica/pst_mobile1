...
class ReportService {
  ...
  Future<Map<String, dynamic>> fetchReports({
    ...
    try {
      ...
      if (response.data != null && response.data['incidents'] != null) {
        ...
        final totalPages =
            response.data['totalPages'] ?? ((total / limit).ceil());

        debugPrint(
          ' [FETCH] ${reports.length} signalements récupérés (page $currentPage/$totalPages)',
        );
  ...
  Future<ReportModel> updateReport({
    ...
    try {
      ...
      if (files != null && files.isNotEmpty) {
        ...
        debugPrint(
          '📎 ${files.length} fichier(s) ajouté(s) - Mode REMPLACEMENT',
        );
      }
  ...
  Future<void> deleteReport(int id, int userId) async {
    try {
      debugPrint('═══════════════════════════════════════════════════════');
      debugPrint(' [DELETE] Début de la suppression');
