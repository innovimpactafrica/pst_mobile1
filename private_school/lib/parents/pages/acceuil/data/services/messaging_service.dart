...
class MessagingService {
  ...
  Future<List<ConversationModel>> getConversations() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Erreur lors de la récupération des conversations: ${response.statusCode}',
        );
      }
    } catch (e, stackTrace) {
  ...
  Future<ConversationModel> createGroupConversation({
    ...
    try {
      ...
      if (memberIds.length < 2) {
        debugPrint(' ATTENTION: Moins de 2 membres sélectionnés!');
  ...
  Future<void> toggleMuteConversation(int conversationId, bool mute) async {
    ...
    try {
      final token = await _storage.getAccessToken();
      final url = Uri.parse(
        '${BaseUrl.current}/api/conversations/$conversationId/mute',
      );
  ...
  Future<void> markConversationAsRead(int conversationId) async {
    ...
    try {
      ...
      debugPrint(' Exception dans markConversationAsRead: $e');
    }
  }
}
