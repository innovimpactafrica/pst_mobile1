...
class TripBloc extends Bloc<TripEvent, TripState> {
  ...
  ///  CHARGER LES TRAJETS DISPONIBLES (avec filtrage des réservés)
  Future<void> _onLoadAvailableTrips(
    ...
    try {
      ...
      // Charger TOUS les trajets depuis l'API
      final allTrips = await repository.getAvailableTrips();

      debugPrint(' Total trajets API: ${allTrips.length}');

      //  Charger les réservations pour savoir quels trajets exclure
      try {
      ...
      debugPrint(
        ' [TripBloc] ${availableTrips.length} trajets disponibles chargés',
      );
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n');

      emit(TripLoaded(trips: availableTrips, selectedTabIndex: 0));
    } catch (e, stackTrace) {
      debugPrint('');
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  ...
  ///  CHARGER MES RÉSERVATIONS (avec mise à jour du cache)
  Future<void> _onLoadMyReservations(
    ...
    try {
      ...
      emit(TripLoaded(trips: reservations, selectedTabIndex: 1));
    } catch (e) {
  ...
  ///  RÉSERVER UN TRAJET (avec mise à jour du cache)
  ...
  ///  ANNULER UNE RÉSERVATION (avec mise à jour du cache)
